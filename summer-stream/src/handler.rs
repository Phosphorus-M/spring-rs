...
/// TypeHandler is used to configure the summer-macro marked stream_listener handler
///
...
macro_rules! submit_typed_handler {
    ($ty:ident) => {
        ::summer_stream::handler::submit! {
            &$ty as &dyn ::summer_stream::handler::TypedHandlerRegistrar
        }
    };
}
